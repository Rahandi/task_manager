...
===============================================*/
    public function view($id)  {
        ...
        //  dd($task[0]->user_id);
        //  dd($id);
        $submit = AssignTask::where('id_user', $task[0]->user_id)->where('task_id',$id)->get() ;
        $nilai = nilaitask::where('user_id', $task[0]->user_id)->where('task_id',$id)->get() ;
        ...
        if ( count($submit) > 0 ) { 
                ...
                // explode the filename into 2 parts: the filename and the extension
                $submit = explode(".", $submit[0]->filename ) ;
                // dd($submit);
                // store images only in one array
                // $submit[0] = filename
                // $submit[1] = jpg
                // check if extension is a image filetype
                if ( in_array($submit[1], $images_array ) ) 
                    $submit_images_set[] = $submit[0] . '.' . $submit[1] ;
                    // if not an image, store in files array
                else
                    $submit_files_set[] = $submit[0] . '.' . $submit[1]; 
        ...
        return view('task.view')
            ...
            ->with('submit_images_set', $submit_images_set)
            ->with('submit_files_set', $submit_files_set) 
            ->with('submit',$submit)
...
===============================================*/
    public function create()
    {
        $id = Auth::user()->id;
        // $projects = Project::all() ;
        $projects = Project::where('id_mgr', $id)->get();
        $users = User::all() ;
    ...
    public function store(Request $request)
    {
        ...
        if($request->user == 'all'){
            ...
            $this->validate( $request, [
                ...
                'task'       => 'required',
                'project_id' => 'required|numeric',
                'photos.*'   => 'sometimes|required|mimes:png,gif,jpeg,jpg,txt,pdf,doc,ppt',  // photos is an array: photos.*
                'duedate'    => 'required'
            ]) ;
            ...
            if( $request->hasFile('photos') ) {
                foreach ($request->photos as $file) {
                   ...
                   // $filename = str_replace(' ' , '' , time() . '_' .$file->getClientOriginalName() );  // get original file name ex:   cat.jpg
                    // remove whitespaces and dots in filenames : [' ' => '', '.' => ''] 
                    $temp = 'file';
                    $filename = strtr( pathinfo( $temp . '_' . $file->getClientOriginalName(), PATHINFO_FILENAME) , [' ' => '', '.' => ''] ) . '.' . pathinfo($file->getClientOriginalName(), PATHINFO_EXTENSION);
                    $file->move('images',$filename);
...
===============================================*/
    ...
    public function submit_nilai(Request $request){
        // dd($request->all());
        $this->validate( $request, [
        ...
        nilaitask::create([
            'task_id'  => $request->task_id,
            'user_id' => $request->user_id , // For Regular Public Images
            'nilai' => $request->nilai
        ]);
        $task_complete = Task::find($request->task_id) ;
        $task_complete->completed = 1;
        $task_complete->save() ;
        Session::flash('success', 'Nilai Berhasil di submit') ;
        return redirect()->back() ;
    }
