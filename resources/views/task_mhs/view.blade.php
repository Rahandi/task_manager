...
<div class="col-md-8">
    ...
    {{-- submit --}}
            ...
            @if( count($submit) >0 ) 
            <div>
                <span class="label label-success">Telah Di Submit</span>
                </div>
            @else
        
            <form action="{{ route('taskmhs.submit') }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                <label><b>Submit Task</b> (png,gif,jpeg,jpg,txt,pdf,doc) <span class="glyphicon glyphicon-file" aria-hidden="true"></span></label>
                <div class="form-grop">
                        
    
                <input type="hidden" name="id_task" value="{{$task_view->id}}">
    ...
    <div class="panel panel-jc">
        ...
        <div class="panel-body">
            @if ( $task_view->completed == 0 )
                <span class="label label-warning">Belum di submit</span>
                @if ( $is_overdue )
                ...
                @endif                
            @else
                <span class="label label-success">Sudah di submit</span>
            @endif
        </div>
    </div>
